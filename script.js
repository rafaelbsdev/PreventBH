function initMap() {
  ...
  map = new google.maps.Map(document.getElementById("map"), {
    zoom: 13,
    center: { lat: -19.905801, lng: -43.988869 },
  });
...
let totalAcidentes = document.getElementById('total-acidentes')
let totalMortes = document.getElementById('total-mortes')

let container = [];
let tempContainer = [];
let salvaLatLngHeatmap = [];
let LatLng = [];
...
const uploadConfirm = document.getElementById('uploadConfirm').addEventListener('click', () => {
  Papa.parse(document.getElementById('uploadFile').files[0], {
    ...
    complete: function (results) {
      for (let i = 1; i < Object.keys(results.data).length; i++) {
        if (
          String(results.data[i][3]).trim() == 'H01003' ||
          String(results.data[i][3]).trim() == 'H02003' ||
          String(results.data[i][3]).trim() == 'H03003' ||
          String(results.data[i][3]).trim() == 'H05003' ||
          String(results.data[i][3]).trim() == 'H06001' ||
          String(results.data[i][3]).trim() == 'H07003' ||
          String(results.data[i][3]).trim() == 'H08003' ||
          String(results.data[i][3]).trim() == 'H09003' ||
          String(results.data[i][3]).trim() == 'H98003'
        ) {
          tempContainer.push({
            'lat': results.data[i][14],
            'lng': results.data[i][15]
          })
        }
        if (String(results.data[i][3]).trim() === 'H01003') {
        }
        //* H01003, H02003, H03003, H05003, H06001, H07003, H08003, H09003, H98003
        container.push({
          'lat': results.data[i][14],
          'lng': results.data[i][15]
        })
      ...
      totalAcidentes.innerHTML = `Total de acidentes: ${container.length}`
      totalMortes.innerHTML = `Total de mortes: ${tempContainer.length}`
      LatLng = tempContainer.map((item) => {
        let kleber = Utm2Wgs(Number(item.lat), Number(item.lng), 23, 'S')
        const [lat, lng] = kleber.split(';');
        return {
          lat: lat.trim(),
          lng: lng.trim()
        }
      ...
      function getPoints() {
        getAllLatLng = container.map((item) => {
          let jorge = Utm2Wgs(Number(item.lat), Number(item.lng), 23, 'S')
          const [lat, lng] = jorge.split(';')

          let temp = {
            lat: Number(lat.trim()),
            lng: Number(lng.trim())
          }
          salvaLatLngHeatmap.push(new google.maps.LatLng(temp.lat, temp.lng));

        });
      }

      heatmap = new google.maps.visualization.HeatmapLayer({
        data: salvaLatLngHeatmap,
        map: map,
      })

      getPoints();
      console.log(salvaLatLngHeatmap.length)
...
function showMarkers() {
  hideMarkers()
  LatLng.forEach((e) => {
    let pos = new google.maps.LatLng(e.lat, e.lng)
    const marker = new google.maps.Marker({
      position: pos,
      map,
      icon: './img/grim-reaper-resized.png',
